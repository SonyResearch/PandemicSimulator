...
from ..environment import Home, Location, CityRegistry, GroceryStore, Road, Cemetery, Hospital, \
    ...
    NonEssentialBusinessLocationState, RetailStore, BarberShop, PopulationParams, Bar, LocationID

__all__ = ['make_standard_locations']


# helper method that encapsulates the addition of bars
def add_bars_restaurants(all_locations: List[Location],
    ...
    all_locations += [Bar(
        ...
        init_state=NonEssentialBusinessLocationState(
            ...
            open_time=SimTimeTuple(hours=tuple([x for x in range(0, 24) if x <= 2 or x >= 21]),
                                   week_days=tuple(range(1, 7)))),
        numpy_rng=numpy_rng
    ) for i in range(location_type_to_params[Bar].num)]

    all_locations += [Restaurant(
        registry=registry,
...
def make_standard_locations(population_params: PopulationParams,
    ...
    add_bars_restaurants(all_locs, population_params, registry, numpy_rng, road.id)
