...
from ..environment import LocationID, PersonRoutine, RepeatablePersonRoutine, \
    Registry, SimTimeInterval, HairSalon, Retired, Restaurant, Bar, SimTimeTuple, GroceryStore, RetailStore, \
    SpecialEndLoc
...
def social_repeatable_routine(start_loc: Optional[LocationID],
...
def get_minor_routines(home_id: LocationID,
    ...
    if age >= 12:
        routines.append(social_repeatable_routine(home_id, numpy_rng))
...
def get_adult_routines(person_type: Type,
    ...
    routines = [
        ...
        weekend_repeatable_routine(registry, None, Restaurant, numpy_rng, explore_probability=0.5)
    ]
    if person_type == Retired:
        routines.append(triggered_repeatable_routine(registry, home_id, Bar, 2, numpy_rng, explore_probability=0.5))
    else:
