...
from .population_params import small_town_population_params, \
    medium_town_population_params, above_medium_town_population_params
from ..data import H5DataSaver, StageSchedule
from ..environment import PandemicSimOpts, PandemicSimNonCLIOpts, PandemicRegulation, Risk, PopulationParams
...
__all__ = ['EvaluationOpts', 'evaluate_strategies', 'evaluate_spread_rates', 'evaluate_location_contact_rates',
           ...
           'population_size_to_params']

population_size_to_params: Dict[int, PopulationParams] = {pp.num_persons: pp for pp in [
...
class EvaluationOpts:
    ...
    default_sim_non_cli_opts: PandemicSimNonCLIOpts = PandemicSimNonCLIOpts(small_town_population_params)
...
def evaluate_strategies(exp_name: str, eval_opts: EvaluationOpts) -> None:
    ...
    for i, strategy in enumerate(eval_opts.strategies):
        ...
        experiment_main(sim_opts=sim_opts,
                        sim_non_cli_opts=eval_opts.default_sim_non_cli_opts,
...
def evaluate_population_sizes(exp_name: str, eval_opts: EvaluationOpts) -> None:
    ...
    for i, population_size in enumerate(eval_opts.population_sizes):
        ...
        experiment_main(sim_opts=PandemicSimOpts(),
                        sim_non_cli_opts=PandemicSimNonCLIOpts(population_size_to_params[population_size]),
                        data_saver=data_saver,
                        num_random_seeds=eval_opts.num_seeds,
                        max_episode_length=eval_opts.max_episode_length,
                        exp_id=i)
